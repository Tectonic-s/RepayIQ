...
class _ResultSectionState extends State<_ResultSection> {
  ...
  Future<void> _share() async {
    ...
    final size = MediaQuery.sizeOf(context); // capture before any await
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (_) => pw.Container(
          ...
          child: pw.Row(
            ...
            children: [
              ...
              pw.Text('EMI Calculation Report',
        ...
        footer: (_) => pw.Container(
          ...
          decoration: pw.BoxDecoration(
            border: pw.Border(top: pw.BorderSide(color: lightGrey, width: 1)),
          ),
          child: pw.Text(
            'Generated by RepayIQ  |  ${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year}',
        ...
        build: (ctx) => [
              ...
              children: [
                ...
                pw.Row(
                  ...
                  children: [
                    ...
                    pw.Text(Formatters.pdfCurrency(r.emi),
          ...
          pw.SizedBox(height: 16),
          // Loan summary table
          pw.Text('Loan Summary',
              style: pw.TextStyle(fontSize: 13, fontWeight: pw.FontWeight.bold, color: tealDark)),
          pw.SizedBox(height: 8),
          pw.Table(
            border: pw.TableBorder.all(color: lightGrey, width: 0.5),
            children: [
              _pdfRow('Principal', Formatters.pdfCurrency(r.principal)),
              _pdfRow('Annual Interest Rate', '${r.rate}% p.a.'),
              _pdfRow('Tenure', Formatters.tenure(r.tenure)),
              _pdfRow('Monthly EMI', Formatters.pdfCurrency(r.emi)),
              _pdfRow('Total Interest', Formatters.pdfCurrency(r.totalInterest)),
              _pdfRow('Total Repayment', Formatters.pdfCurrency(r.totalRepayment)),
              if (r.moratoriumInterest != null && r.moratoriumInterest! > 0)
                _pdfRow('Moratorium Interest', Formatters.pdfCurrency(r.moratoriumInterest!)),
              if (r.effectiveRate != null)
                _pdfRow('Effective Rate (No-Cost EMI)', Formatters.percentage(r.effectiveRate!)),
            ],
          ),
          pw.SizedBox(height: 20),
          // Amortisation schedule
          pw.Text('Amortisation Schedule',
              style: pw.TextStyle(fontSize: 13, fontWeight: pw.FontWeight.bold, color: tealDark)),
          pw.SizedBox(height: 8),
          pw.Table(
            border: pw.TableBorder.all(color: lightGrey, width: 0.5),
            ...
            children: [
              pw.TableRow(
                ...
                children: [
                  _pdfCell('Mo.', isHeader: true),
                  _pdfCell('EMI', isHeader: true),
              ...
              ...schedule.asMap().entries.map((e) {
                final row = e.value;
                final bg = e.key.isEven ? PdfColors.white : PdfColor.fromHex('#F7F9FC');
                return pw.TableRow(
                  decoration: pw.BoxDecoration(color: bg),
                  children: [
                    _pdfCell('${row['month']!.toInt()}'),
                    _pdfCell(Formatters.pdfCurrency(row['emi']!)),
                    _pdfCell(Formatters.pdfCurrency(row['principal']!)),
                    _pdfCell(Formatters.pdfCurrency(row['interest']!)),
                    _pdfCell(Formatters.pdfCurrency(row['balance']!)),
                  ],
                );
              }),
            ],
    ...
    final file = File('${dir.path}/RepayIQ_EMI_${r.loanType.label.replaceAll(' ', '_')}.pdf');
    await file.writeAsBytes(bytes);

    // Use screen bounds center as share anchor (required on iPad)
            ...
            Row(children: [
              Expanded(
                child: _ActionBtn(
                  ...
                  color: color,
                  onTap: () async => setState(() => _showSchedule = !_showSchedule),
                ),
              ),
                    ...
                    return Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                          flex: 2,
                          child: Text(
                            Formatters.pdfCurrency(row['principal']!),
                        ...
                        Expanded(
                          flex: 2,
                          child: Text(
                            Formatters.pdfCurrency(row['interest']!),
                        ...
                        Expanded(
                          flex: 2,
                          child: Text(
                            Formatters.pdfCurrency(row['balance']!),
